<script lang="ts">
	...
	const opacity = tweened(0, { duration: 3000 });

	type Interpolator = (id: number, t: number) => string;

	let interpolator: Interpolator = (_: number, __: number) =>
	...
	const onWorkerMessage = (e: {  data: {msg: string, t: number; id: number; path: string } }) => {
		...
		switch (d.msg) {
			case 'loaded':
				...
				break;
			default:
				currentPaths.set(d.id, d.path);
				break;
		}
		
	};

	let currentPaths = new Map<number, string>();
	...
	onMount(async () => {
		loadWorker().then(async () => {
			interpolationWorker!.postMessage({ task: 'load', paths: startPaths });
			interpolator = (id:number, t: number):string => {
				interpolationWorker!.postMessage({ task: 'interpolate', t , id});
