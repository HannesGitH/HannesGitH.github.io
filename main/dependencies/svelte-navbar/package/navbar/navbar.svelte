...
import InterpolationWorker from '../workers/interpolator.worker?worker';
...
let interpolator = (p) => paths.bottom;
let interpolationWorker = undefined;
...
$: elemtsrowStyle =
    scrollProgress_ <= 0
        ...
        : scrollProgress_ >= 1
            ...
            : 'overflow:hidden';
let slotti = null;
const updateSlot = async (scrollP) => {
    //timeout promise
    ...
    if (slotti) {
        ...
        const calcXY = (index, progress, x, y) => {
